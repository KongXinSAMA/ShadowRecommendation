...
from Utils.BaseModel import VAE
from Utils.Dataloader import Base_TrainingDataset
device = "cuda" if torch.cuda.is_available() else "cpu"

def Vae_loss_function(x, x_hat, mean, log_var, anneal=1.):
...
def user_shadow_train_eval(config):
    ...
    train_dataset = Base_TrainingDataset(train_data)
    ...
    for epoch in range(epochs):
        ...
        total_len = 0
        total_loss = 0
        for o in train_dataloader:

            o_hat, mean, log_var = model(o)
            ...
            Vae_loss = Vae_loss_function(o, o_hat, mean, log_var, anneal)
            ...
            anneal_count += 1
            total_len += len(o)
            total_loss += loss.item() * len(o)

        model.eval()
        test_o_hat, mean_val, log_var_val = model(val_data)
        val_loss = Vae_loss_function(val_data, test_o_hat, mean_val, log_var_val, anneal=anneal_max)

        patience_count += 1
        if val_loss < best_val_loss:
            patience_count = 0
            best_val_loss = val_loss
            torch.save(model.state_dict(), data_params["vae_path"] + "{}_{}_Best_vae.pt".format(data_params["name"], "Val"))
        ...
        if config["show_log"]:
            ...
            print("Epoch {}, Training Los"
                  "s = {}, Val Loss = {} ".format(epoch, training_loss, val_loss))
    ...
    # save to local
    if config['save_mean']:
        model.load_state_dict(torch.load(data_params["vae_path"] + "{}_{}_Best_vae.pt".format(data_params["name"], "Val")))
        fullmatrix = torch.tensor(fullmatrix > 0, dtype=torch.float).to(device)
        O_hat, mean, log_var = model(fullmatrix)
        torch.save(mean.detach(), data_params["vae_path"] + "{}_user_mean.pt".format(data_params["name"]))
        torch.save(log_var.detach().exp().sqrt(), data_params["vae_path"] + "{}_user_std.pt".format(data_params["name"]))
