...
from Utils.BaseModel import DeepCausal
device = "cuda" if torch.cuda.is_available() else "cpu"

def Evaluate(data_loader, test_model, user_feat, device="cpu", k=5):
    test_model.eval()
    with torch.no_grad():
        ...
        for index, (uid, iid, rating) in enumerate(data_loader):
            ...
            feat = user_feat[uid.type(torch.long)]
            predict = test_model(uid, iid, feat).view(-1)
            uids.extend(uid.cpu())
            iids.extend(iid.cpu())
...
def train_eval(config):
    ...
    seed_everything(config["seed"])
    user_vae_mean = torch.load(data_params["vae_path"] + "{}_user_mean.pt".format(data_params["name"])).to(device)
    user_vae_std = torch.load(data_params["vae_path"] + "{}_user_mean.pt".format(data_params["name"])).to(device)
    user_feat = torch.tensor(pd.read_csv(data_params["user_feature_label_no"]).to_numpy()).to(device)

    model = DeepCausal(n_users, n_items, data_params['user_feature_dim'], config["embedding_dim"], vae_mean=user_vae_mean, vae_std=user_vae_std, device=device).to(device)
    ...
    for epoch in range(config["epochs"]):
        ...
        model.eval()
        validation_performance = Evaluate(val_loader, model, user_feat, device=device)
        test = Evaluate(test_loader, model, user_feat, device=device)
        if config['show_log']:
...
if __name__ == '__main__':
    ...
    if args.tune:
        ...
        if args.data_params["name"] == 'coat':
            ...
            test_param = {'lr': 1e-05, 'weight_decay': 1e-05, 'embedding_dim': 512}
        elif args.data_params["name"] == 'sim_data':
            test_param = {'lr': 0.00005, 'weight_decay': 1e-05, 'embedding_dim': 256}
        lisT = []
        for i in range(10):
            random_seed = random.randint(1000, 2000)
            Train_eval_config = {
